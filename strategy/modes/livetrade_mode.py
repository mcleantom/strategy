...
class LiveTrader:
    ...
    def should_exit_position(self, candle):
        ...
        if self.stop_loss and (
                (self.position.type == "long" and current_price <= self.stop_loss) or
                (self.position.type == "short" and current_price >= self.stop_loss)
        ):
            return True
        if self.take_profit and (
                (self.position.type == "long" and current_price >= self.take_profit) or
                (self.position.type == "short" and current_price <= self.take_profit)
        ):
            return True
