...
@pytest.fixture
async def exchange() -> AsyncGenerator[None, AlpacaExchange]:
...
async def test_market_order(exchange: AlpacaExchange):
    with aioresponses() as m:
        m.post(f"{BASE}/orders", payload={"id": "ord_market_1"})
        oid = await exchange.market_order("AAPL", 1, 150.0, "buy", False)
...
async def test_limit_order():
    with aioresponses() as m:
        ...
        oid = await exchange.limit_order("AAPL", 1, 145.0, "buy", False)
...
async def test_stop_order():
    with aioresponses() as m:
        ...
        oid = await exchange.stop_order("AAPL", 1, 140.0, "sell", False)
        assert oid == "ord_stop_3"


async def test_cancel_all_orders(exchange: AlpacaExchange):
    with aioresponses() as m:
...
async def test_get_balance(exchange: AlpacaExchange):
    with aioresponses() as m:
...
async def test_fetch_precisions(exchange: AlpacaExchange):
    with aioresponses() as m:
...
async def test_400_response(exchange: AlpacaExchange):
    with aioresponses() as m:
