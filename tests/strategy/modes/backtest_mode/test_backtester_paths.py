...
class ShortOnlyStrategy(Strategy):
    ...
    def go_short(self) -> Order:
        ...
        return Order(quantity=1.0, price=price, stop_loss=price + 100.0, take_profit=price - 10.0)
...
def test_warmup_path_and_short_flow_with_exit():
    ...
    bt = Backtester(strategy=strat, initial_balance=1_000.0, timeframe=ETimeframe.MINUTE_1)
    bt.backtest(candles)
...
def test_should_exit_conditions_long_and_short():
    ...
    class LongStrategy(Strategy):
        ...
        def go_long(self) -> Order:
            ...
            price = float(self.candles[-1]["close"])
            return Order(quantity=1.0, price=price, stop_loss=price - 1.0, take_profit=price + 1.0)
